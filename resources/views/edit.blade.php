...
@section("breadcrumbs")
    ...
    <li>
        <a href="{{action('\Pinerp\Notes\NotesController@create')}}">{{ trans('notes::notes.edit') }}</a>
    </li>
@endsection
...
@section('content')
    ...
    <div class="row">
        <div class="col-md-12">
            <h3>{{ trans('notes::notes.edit') }} - "{{ $note->title }}"</h3>
            <br>
            ...
            {!! Form::model($note, [
                'route' => ['erp.notes.update', $note->id],
                'method' => 'patch'
            ]) !!}
            <div class="form-group row">
                {!! Form::label('title', trans('notes::notes.fields.title'), ['class' => 'col-sm-4 form-control-label require_label']) !!}
                <div class="col-sm-8 input-group">
                    {!! Form::text('title', $note->title, ['required' => 'required', 'class' => 'form-control', 'placeholder' => trans('notes::notes.fields.title')]) !!}
                </div>
            </div>
            ...
            <div class="form-group row">
                ...
                <div class="col-sm-8 input-group">
                    {!! Form::textarea('content', $note->content, ['class' => 'form-control', 'rows' => '3', 'placeholder' => trans('notes::notes.fields.content')]) !!}
                </div>
            </div>

            @if(can('notes.note.create_notify'))
            <div class="form-group row">
                <label for="notify_datetime" class="col-sm-4 form-control-label">{{ trans('notes::notes.fields.notify_time') }}
                    {{--<br><span class="alert alert-warning">Напоминание придёт по смс</span>--}}
                </label>
                <div class='col-sm-2 input-group date'>
                    {!! Form::text('notify_datetime', $note->notify_datetime, ['id' => 'notify_datetime','class' => 'form-control', 'placeholder' => trans('notes::notes.fields.set_notify_time')]) !!}
            ...
            <div class="form-group row">
                <div class="col-sm-3 col-sm-offset-4 input-group">
                    {!! Form::submit(trans('layout::common.button.edit'), ['class' => 'btn btn-success']) !!}
                </div>
            </div>
